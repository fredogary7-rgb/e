...
class DailyTask(db.Model):
    """Tâche quotidienne - produit ou publicité à partager"""
...
class UserTask(db.Model):
    """Suivi du partage d'une tâche par un utilisateur"""
    __tablename__ = 'user_tasks'
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id', ondelete='CASCADE'), nullable=False)
    task_id = db.Column(db.Integer, db.ForeignKey('daily_tasks.id', ondelete='CASCADE'), nullable=False)
    shared = db.Column(db.Boolean, default=False)
    shared_at = db.Column(db.DateTime)

    user = db.relationship('User', backref=db.backref('user_tasks', lazy='dynamic'))
    task = db.relationship('DailyTask', backref=db.backref('user_tasks', lazy='dynamic'))

    def __repr__(self):
        return f'<UserTask user={self.user_id} task={self.task_id} shared={self.shared}>'


class TaskReward(db.Model):
    """Récompense journalière pour avoir complété les 10 tâches"""
    ...
    user = db.relationship('User', backref=db.backref('task_rewards', lazy='dynamic'))
...
# ═══════════════════════════════════════════════════════════
# 🎯 TÂCHES QUOTIDIENNES - PARTAGE DE PUBLICITÉS RÉMUNÉRÉ
# ═══════════════════════════════════════════════════════════
...
def can_do_tasks_today(user_id, date_obj):
    reward = TaskReward.query.filter_by(user_id=user_id, date=date_obj).first()
    if reward:
...
def taches_page():
    ...
    if not user_is_activated(user):
        flash("Compte non activé. Faites un premier dépôt.", "warning")
        return redirect(url_for('dashboard_page'))
...
def api_share_task():
    ...
    today = now.date()
    if now.weekday() >= 5:
        return jsonify({'success': False, 'message': 'Week-end'}), 400
    if now.hour < 8 or now.hour >= 23 or (now.hour == 23 and now.minute > 59):
        return jsonify({'success': False, 'message': 'Hors horaires'}), 400

    task = DailyTask.query.get(task_id)
    if not task or not task.actif or task.date != today:
        return jsonify({'success': False, 'message': 'Tâche invalide'}), 400

    existing = UserTask.query.filter_by(user_id=user.id, task_id=task_id).first()
    if existing and existing.shared:
        return jsonify({'success': False, 'message': 'Déjà partagé'}), 400
    ...
    db.session.commit()
    sc, _, _ = get_user_task_progress(user.id, today)
...
def api_claim_task_reward():
    ...
    sc, _, _ = get_user_task_progress(user.id, today)
    if sc < TASK_COUNT:
        return jsonify({'success': False, 'message': f'Encore {TASK_COUNT - sc} partage(s) à faire'}), 400

    import random
    montant = random.randint(TASK_REWARD_MIN, TASK_REWARD_MAX)
    user.solde_parrainage = (user.solde_parrainage or 0) + montant
    db.session.add(TaskReward(user_id=user.id, date=today, montant=montant))
    db.session.commit()
    return jsonify({'success': True, 'montant': int(montant), 'message': f'Félicitations ! +{int(montant)} FCFA 🎉'})
...
def admin_taches():
    ...
    if request.method == 'POST':
        ...
        if action == 'generate':
            ...
            flash(f"{len(tasks)} tâches générées.", "success")
        elif action == 'toggle':
            task = DailyTask.query.get(request.form.get('task_id'))
            if task:
                task.actif = not task.actif
                db.session.commit()
                flash(f"Tâche #{task.id} {'activée' if task.actif else 'désactivée'}.", "success")
        elif action == 'regenerate':
            DailyTask.query.filter_by(date=today).delete()
            db.session.commit()
            get_or_create_daily_tasks(today)
            flash("Tâches régénérées.", "success")
    ...
    vendeur_stats = db.session.query(Boutique.nom, sa_func.count(UserTask.id).label('total')) \
        ...
        .filter(UserTask.shared == True).group_by(Boutique.id) \
        .order_by(sa_func.count(UserTask.id).desc()).limit(20).all()
...
def select_daily_content(date_obj):
    produits = Produit.query.filter_by(est_actif=True).order_by((Produit.vues+Produit.ventes*10).desc()).limit(7).all()
    pubs = Publicite.query.filter_by(est_actif=True).order_by((Publicite.vues+Publicite.partages*5).desc()).limit(5).all()
    items = [('produit',p.id) for p in produits] + [('publicite',pub.id) for pub in pubs]
...
def get_or_create_daily_tasks(date_obj):
    tasks = DailyTask.query.filter_by(date=date_obj,actif=True).order_by(DailyTask.ordre).all()
    if not tasks:
        for i,(ctype,cid) in enumerate(select_daily_content(date_obj)):
            if ctype=='produit': t=DailyTask(produit_id=cid,content_type='produit',date=date_obj,ordre=i)
            else: t=DailyTask(publicite_id=cid,content_type='publicite',date=date_obj,ordre=i)
            db.session.add(t)
        db.session.commit()
        tasks = DailyTask.query.filter_by(date=date_obj,actif=True).order_by(DailyTask.ordre).all()
    return tasks
...
def taches_page():
    ...
    if now.weekday()>=5:
        return render_template('taches_clean.html',user=user,taches=[],shared_count=0,total=TASK_COUNT,can_start=False,message="⏸️ Lun-Ven uniquement.")
    if now.hour<8:
        return render_template('taches_clean.html',user=user,taches=[],shared_count=0,total=TASK_COUNT,can_start=False,message="⏰ Ouvre à 08h00.")
    if now.hour>=23:
        return render_template('taches_clean.html',user=user,taches=[],shared_count=0,total=TASK_COUNT,can_start=False,message="🌙 Terminé. Revenez demain!")
    # Vérifier récompense du jour
    rw = TaskReward.query.filter_by(user_id=user.id,date=today).first()
    if rw:
        return render_template('taches_clean.html',user=user,taches=[],shared_count=TASK_COUNT,total=TASK_COUNT,can_start=False,reward_amount=rw.montant,message=f"✅ +{int(rw.montant)} FCFA aujourd'hui !")
    tasks = get_or_create_daily_tasks(today)
    sc,utm,_ = get_task_progress(user.id,today)
    td=[]
    for task in tasks:
        ut = utm.get(task.id)
...
def api_share_task():
    ...
    if not task_id: return jsonify({'success':False,'message':'task_id requis'}),400
    now = datetime.now(); today = now.date()
    ...
    if existing and existing.shared:
        return jsonify({'success':False,'message':'Déjà partagé'}),400
    if existing:
        existing.shared=True; existing.shared_at=datetime.utcnow()
    else:
        db.session.add(UserTask(user_id=user.id,task_id=task_id,shared=True,shared_at=datetime.utcnow()))
    # Incrémenter compteur publicité
    pub = Publicite.query.filter_by(produit_id=task.produit_id).first()
    if pub: pub.partages=(pub.partages or 0)+1
    db.session.commit()
    sc,_,_ = get_task_progress(user.id,today)
    return jsonify({'success':True,'shared_count':sc,'total':TASK_COUNT,'completed':sc>=TASK_COUNT})

@app.route('/api/claim-task-reward',methods=['POST'])
def api_claim_task_reward():
    user = get_logged_in_user()
    if not user: return jsonify({'success':False,'message':'Non connecté'}),401
    today = datetime.now().date()
    if TaskReward.query.filter_by(user_id=user.id,date=today).first():
        return jsonify({'success':False,'message':'Déjà récompensé'}),400
    sc,_,_ = get_task_progress(user.id,today)
    if sc<TASK_COUNT:
        return jsonify({'success':False,'message':f'Encore {TASK_COUNT-sc} partage(s)'}),400
    import random
    montant = random.randint(TASK_REWARD_MIN,TASK_REWARD_MAX)
    user.solde_parrainage = (user.solde_parrainage or 0)+montant
    db.session.add(TaskReward(user_id=user.id,date=today,montant=montant))
    db.session.commit()
    return jsonify({'success':True,'montant':int(montant),'message':f'+{int(montant)} FCFA 🎉'})

@app.route('/admin/taches',methods=['GET','POST'])
def admin_taches():
    user = get_logged_in_user()
    if not user or not user.is_admin:
        flash("Accès refusé.","danger"); return redirect(url_for('connexion_page'))
    today = date.today()
    if request.method=='POST':
        action = request.form.get('action')
        if action=='generate':
            get_or_create_daily_tasks(today); flash("Tâches générées.","success")
        elif action=='toggle':
            t=DailyTask.query.get(request.form.get('task_id'))
            if t: t.actif=not t.actif; db.session.commit(); flash(f"Tâche #{t.id} {'activée' if t.actif else 'désactivée'}.","success")
        elif action=='regenerate':
            DailyTask.query.filter_by(date=today).delete(); db.session.commit()
            get_or_create_daily_tasks(today); flash("Régénérées.","success")
        return redirect(url_for('admin_taches'))
    tasks = DailyTask.query.filter_by(date=today).order_by(DailyTask.ordre).all()
    for t in tasks: t.share_count = UserTask.query.filter_by(task_id=t.id,shared=True).count()
    from sqlalchemy import func as sa_func
    vs = db.session.query(Boutique.nom,sa_func.count(UserTask.id).label('total'))\
        .join(Produit,Produit.boutique_id==Boutique.id)\
        .join(DailyTask,DailyTask.produit_id==Produit.id)\
        .join(UserTask,UserTask.task_id==DailyTask.id)\
        .filter(UserTask.shared==True).group_by(Boutique.id)\
    ...
    return render_template('admin_taches.html',user=user,tasks=tasks,today=today,vendeur_stats=vs)
            ...
            td.append({'task_id':task.id,'type':'publicite','nom':pub.titre,'image':pub.video_url,'boutique_nom':pub.boutique.nom if pub.boutique else 'NovaTrade','shared':ut.shared if ut else False})
        else:
            p=task.produit
            td.append({'task_id':task.id,'type':'produit','nom':p.nom if p else '?','image':(p.liste_images[0] if p and p.liste_images else None),'boutique_nom':p.boutique.nom if p and p.boutique else 'NovaTrade','shared':ut.shared if ut else False})
    import random
...
def get_task_progress(user_id,date_obj):
    tasks_today = DailyTask.query.filter_by(date=date_obj,actif=True).order_by(DailyTask.ordre).all()
    task_ids = [t.id for t in tasks_today]
    uts = UserTask.query.filter(UserTask.user_id==user_id,UserTask.task_id.in_(task_ids)).all()
    return sum(1 for u in uts if u.shared), {u.task_id:u for u in uts}, tasks_today
...
def get_user_task_progress(user_id, date_obj):
    tasks_today = DailyTask.query.filter_by(date=date_obj, actif=True).order_by(DailyTask.ordre).all()
    task_ids = [t.id for t in tasks_today]
    user_tasks = UserTask.query.filter(UserTask.user_id==user_id, UserTask.task_id.in_(task_ids)).all()
    shared_count = sum(1 for ut in user_tasks if ut.shared)
    return shared_count, {ut.task_id: ut for ut in user_tasks}, tasks_today
